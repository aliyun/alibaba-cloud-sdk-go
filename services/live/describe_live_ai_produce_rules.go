...
func (client *Client) DescribeLiveAIProduceRules(request *DescribeLiveAIProduceRulesRequest) (response *DescribeLiveAIProduceRulesResponse, err error) {
	response = CreateDescribeLiveAIProduceRulesResponse()
...
func (client *Client) DescribeLiveAIProduceRulesWithChan(request *DescribeLiveAIProduceRulesRequest) (<-chan *DescribeLiveAIProduceRulesResponse, <-chan error) {
	responseChan := make(chan *DescribeLiveAIProduceRulesResponse, 1)
	errChan := make(chan error, 1)
	err := client.AddAsyncTask(func() {
		defer close(responseChan)
		defer close(errChan)
		response, err := client.DescribeLiveAIProduceRules(request)
		if err != nil {
			errChan <- err
		} else {
...
func (client *Client) DescribeLiveAIProduceRulesWithCallback(request *DescribeLiveAIProduceRulesRequest, callback func(response *DescribeLiveAIProduceRulesResponse, err error)) <-chan int {
	...
	err := client.AddAsyncTask(func() {
		var response *DescribeLiveAIProduceRulesResponse
		var err error
		defer close(result)
		response, err = client.DescribeLiveAIProduceRules(request)
...
type DescribeLiveAIProduceRulesRequest struct {
	*requests.RpcRequest
	...
	SuffixName string           `position:"Query" name:"SuffixName"`
...
type DescribeLiveAIProduceRulesResponse struct {
	*responses.BaseResponse
	...
	RuleInfoList RuleInfoListInDescribeLiveAIProduceRules `json:"RuleInfoList" xml:"RuleInfoList"`
}
...
func CreateDescribeLiveAIProduceRulesRequest() (request *DescribeLiveAIProduceRulesRequest) {
	request = &DescribeLiveAIProduceRulesRequest{
		RpcRequest: &requests.RpcRequest{},
	}
	request.InitWithApiInfo("live", "2016-11-01", "DescribeLiveAIProduceRules", "live", "openAPI")
...
func CreateDescribeLiveAIProduceRulesResponse() (response *DescribeLiveAIProduceRulesResponse) {
	response = &DescribeLiveAIProduceRulesResponse{
