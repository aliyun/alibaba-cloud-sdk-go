...
func (client *Client) BbsPrevention(request *BbsPreventionRequest) (response *BbsPreventionResponse, err error) {
	response = CreateBbsPreventionResponse()
	err = client.DoAction(request, response)
	return
}

func (client *Client) BbsPreventionWithChan(request *BbsPreventionRequest) (<-chan *BbsPreventionResponse, <-chan error) {
	responseChan := make(chan *BbsPreventionResponse, 1)
	errChan := make(chan error, 1)
	err := client.AddAsyncTask(func() {
		defer close(responseChan)
		defer close(errChan)
		response, err := client.BbsPrevention(request)
...
func (client *Client) BbsPreventionWithCallback(request *BbsPreventionRequest, callback func(response *BbsPreventionResponse, err error)) <-chan int {
	result := make(chan int, 1)
	err := client.AddAsyncTask(func() {
		var response *BbsPreventionResponse
		var err error
		defer close(result)
		response, err = client.BbsPrevention(request)
...
func CreateBbsPreventionRequest() (request *BbsPreventionRequest) {
	request = &BbsPreventionRequest{
		RpcRequest: &requests.RpcRequest{},
	}
	request.InitWithApiInfo("jaq", "2017-08-25", "BbsPrevention", "", "")
	return
}

func CreateBbsPreventionResponse() (response *BbsPreventionResponse) {
	response = &BbsPreventionResponse{
